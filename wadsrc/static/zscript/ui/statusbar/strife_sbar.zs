...
class StrifeStatusBar : BaseStatusBar
{
	...
	override void Tick ()
	{
		...
		if (PendingPop != POP_NoChange)
		{
			if (PopHeight < 0)
			{
				PopHeightChange = POP_HEIGHT / (GameTicRate / 8);
				PopHeight += POP_HEIGHT / (GameTicRate / 8);
			}
			else
			{
		...
		else
		{
			...
			if (KeyPopScroll > 0)
			{
				KeyPopScroll -= 280 / (GameTicRate / 3);
				if (KeyPopScroll < 0)
				{
					KeyPopScroll = 0;
				}
	...
	protected void DrawPopScreen (int bottom, double TicFrac)
	{
		...
		case POP_Keys:
			...
			if (KeyPopScroll > 0)
			{
				...
				int scroll = MAX (0, KeyPopScroll - int(TicFrac * (280./(GameTicRate / 3))));
				pos -= 10;
